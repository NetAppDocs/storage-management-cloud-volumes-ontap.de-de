...
Sie können eine Anwendung verwenden, um Dateien über NFS oder CIFS an WORM zu übertragen, oder die ONTAP CLI verwenden, um Dateien automatisch an WORM zu übertragen. Sie können auch eine WORM-anhängbare Datei verwenden, um Daten zu speichern, die inkrementell geschrieben werden, wie Protokollinformationen. Weitere Informationen finden Sie unter https://docs.netapp.com/us-en/ontap/snaplock/commit-files-worm-state-manual-task.html#commit-files-to-worm-manually["Dateien mit ONTAP SnapLock auf WORM festschreiben"^].
