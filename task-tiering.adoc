...
---
= Tieren Sie inaktive Cloud Volumes ONTAP Daten in einen kostengünstigen Objektspeicher
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* Für Azure müssen Sie nichts tun, solange die NetApp Konsole über die erforderlichen Berechtigungen verfügt. <<Anforderungen zum Tiering kalter Daten in Azure Blob Storage,Mehr erfahren>> .
...
[role="quick-margin-para"]
Die NetApp Konsole fordert Sie auf, eine Tiering-Richtlinie auszuwählen, wenn Sie ein Volume erstellen, ändern oder replizieren.
...
* *Vorhandene Bände*
+
Um Data Tiering auf einem vorhandenen Volume zu aktivieren, stellen Sie sicher, dass es auf dem zugrunde liegenden Aggregat aktiviert ist.  Wenn die Datenaufteilung auf dem vorhandenen Aggregat nicht aktiviert ist, müssen Sie ONTAP System Manager verwenden, um ein vorhandenes Aggregat an den Objektspeicher anzuhängen.
