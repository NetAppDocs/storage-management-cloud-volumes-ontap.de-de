...
Beachten Sie, dass ein optionales Management-LIF für eine Storage-VM eine private IP-Adresse auf einem Einzelknotensystem und auf einem HA-Paar in einer einzelnen AZ benötigt. Dieses Management-LIF stellt eine Verbindung zu Management-Tools wie SnapCenter bereit.
...
== Erstellen Sie eine Storage-VM auf einem Einzelknotensystem

Diese Schritte erstellen eine neue Storage-VM auf einem Einzelknotensystem. Eine private IP-Adresse ist erforderlich, um eine NAS-LIF zu erstellen, und eine weitere optionale private IP-Adresse wird benötigt, wenn Sie eine Management-LIF erstellen möchten.
...
** `-service-policy default-data-iscsi` weist darauf hin, dass eine IP-Adresse nicht zwischen Knoten migriert wird.
