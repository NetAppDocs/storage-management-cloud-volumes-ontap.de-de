...
* Eine einzelne Verfügbarkeitszone (AZ) oder mehrere Verfügbarkeits-/drei Zonen (empfohlen).
+
...
Traditionell verwenden Cloud Volumes ONTAP HA Konfigurationen in Google Cloud ein nicht-gemeinsam genutztes Modell, bei dem eine Mediator-Instanz zur Verwaltung von Failover-Vorgängen oder zur Überwachung des Knotenzustands eingesetzt wird. Dies erfordert das Erstellen einer zusätzlichen VM in Google Cloud und zwei Festplattensätze für jeden Knoten.
...
Zugriff auf die Instanz:: Für Debian ist der Standard-Cloud-Benutzer  `admin`. Google Cloud erstellt und fügt ein Zertifikat für den  `admin` Benutzer hinzu, wenn der SSH-Zugriff über die Google Cloud Console oder die gcloud Befehlszeile angefordert wird. Sie können  `sudo` angeben, um Root-Rechte zu erhalten.
...
=== Gemeinsam genutzte Datenträger für Single AZ HA-Konfigurationen

Cloud Volumes ONTAP 9.19.1 und höher unterstützen die gemeinsame HA-Konfiguration für eine einzelne AZ. In der gemeinsamen HA-Architektur können beide Knoten eine Festplatte gemeinsam nutzen, wodurch sich die Methode zur Aggregaterstellung ändert. Anstatt zwei Festplatten für zwei VMs zu erstellen, wird eine einzelne Festplatte erstellt und den VMs zugeordnet. Die gemeinsame HA-Konfiguration benötigt kein Festplatten-Mirroring.
...
Mehrere Verfügbarkeitszonen (empfohlen):: Durch die Bereitstellung einer HA-Konfiguration über drei Zonen hinweg wird eine kontinuierliche Datenverfügbarkeit gewährleistet, wenn innerhalb einer Zone ein Fehler auftritt.  Beachten Sie, dass die Schreibleistung im Vergleich zur Verwendung einer einzelnen Zone etwas geringer ist, aber minimal.
Einzelne Verfügbarkeitszone:: Bei der Bereitstellung in einer einzelnen Verfügbarkeitszone (AZ) verwendet eine Cloud Volumes ONTAP HA Konfiguration eine verteilte Platzierungsrichtlinie. Diese Richtlinie stellt sicher, dass eine HA Konfiguration vor einem Single Point of Failure innerhalb der Zone geschützt ist, ohne dass separate Zonen zur Fehlerisolierung erforderlich sind.
+
--
Dieses Bereitstellungsmodell senkt Ihre Kosten, da zwischen den Zonen keine Gebühren für den Datenausgang anfallen.

== Wie die Speicherung in einem HA Pair funktioniert
