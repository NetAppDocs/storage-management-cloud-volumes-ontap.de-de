...
Während der Bereitstellung von Cloud Volumes ONTAP akzeptiert das Feld für den AWS-Mediator-Proxy eine Serveradresse. Wenn Sie eine vollqualifizierte Domain (FQDN) anstelle einer IP-Adresse eingeben, kann der FQDN nicht aufgelöst werden, es sei denn, die AWS VPC ist mit einem benutzerdefinierten DNS-Server konfiguriert. Dies liegt daran, dass der standardmäßig von AWS bereitgestellte DNS-Server nur öffentliche DNS-Einträge und interne AWS-Hosts auflöst und keine externen FQDNs wie Proxyserver von Kunden auflösen kann. Wenn dies auf Sie zutrifft, haben Sie zwei Möglichkeiten:

* Fügen Sie die IP-Adresse des Proxyservers direkt in das Feld „Mediator-Proxy“ ein, anstatt die vollqualifizierte Domain (FQDN) zu verwenden.
* Konfigurieren Sie in Ihrer VPC einen DHCP-Optionssatz, der Ihre benutzerdefinierten DNS-Serveradressen enthält. Dadurch kann AWS die vollqualifizierte Domain (FQDN) auflösen, wenn Sie diese während der Bereitstellung im Feld Proxyserver eingeben. Weitere Informationen finden Sie in der  https://docs.aws.amazon.com/vpc/latest/userguide/DHCPOptionSet.html["AWS-Dokumentation"^].
...
Die Anzahl der LIFs, die die Console für Cloud Volumes ONTAP bereitstellt, hängt davon ab, ob Sie ein Single-Node-System oder ein HA-Paar bereitstellen. Ein LIF ist eine IP-Adresse, die einem physischen Port zugeordnet ist.
...
HA-Paare benötigen mehr IP-Adressen als ein Einzelknotensystem. Diese IP-Adressen sind auf verschiedene Ethernet-Schnittstellen verteilt, wie in der folgenden Abbildung dargestellt:
...
Wenn Sie EBS als Performance-Tier und Amazon S3 als Capacity-Tier verwenden möchten, müssen Sie sicherstellen, dass Cloud Volumes ONTAP eine Verbindung zu S3 hat. Die beste Möglichkeit, diese Verbindung bereitzustellen, ist die Erstellung eines VPC-Endpunkts für den S3-Dienst. Anweisungen finden Sie unter  https://docs.aws.amazon.com/AmazonVPC/latest/UserGuide/vpce-gateway.html#create-gateway-endpoint["AWS-Dokumentation: Erstellen eines Gateway-Endpunkts"^].
