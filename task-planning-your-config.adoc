...
NOTE: Data Tiering zu Amazon Simple Storage Service (Amazon S3) wird nicht unterstützt, wenn sich Ihr Cloud Volumes ONTAP System in einer AWS Local Zone befindet, da der Zugriff auf die Amazon S3 Buckets außerhalb der Local Zone eine höhere Latenz verursacht und die Aktivitäten von Cloud Volumes ONTAP beeinträchtigt.
