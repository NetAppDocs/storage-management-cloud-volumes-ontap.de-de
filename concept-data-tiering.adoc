...
--
Speicherklassen:: Google Cloud verwendet  https://cloud.google.com/storage/docs/storage-classes["Speicherklassen"^] zur Verwaltung von Kosten und Datenverfügbarkeit. Es wird empfohlen, die Speicherklasse entsprechend der erwarteten Datenzugriffshäufigkeit auszuwählen, da der Datenzugriff mit höheren Kosten verbunden ist. Die Standard Storage-Klasse für gestaffelte Daten ist _Standard Storage_. Sofern nicht manuell geändert, behalten alle inaktiven Daten die Standard Storage-Klasse bei. Zum Ändern der Speicherklasse inaktiver Daten siehe  https://docs.cloud.google.com/storage/docs/managing-lifecycles["Google Cloud Dokumentation: Verwaltung von Objektlebenszyklen"^].
+
--
Die Speicherklasse für die Datenschichtung gilt systemweit, nicht pro Datenträger.
