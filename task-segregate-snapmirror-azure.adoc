...
---
= Den SnapMirror-Datenverkehr in Cloud Volumes ONTAP in Azure segregieren
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
----
.. Speichern Sie die privateIPAddress. Diese IP-Adresse, <new_added_nic_primary_addr>, wird verwendet, um einen Intercluster-LIF in <<step-second,Broadcast-Domäne und Intercluster-LIF für die neue Netzwerkkarte>> zu erstellen.
...
== [[step-second]]Schritt 2: Erstellen Sie einen neuen IPspace, eine Broadcast-Domäne und eine Intercluster-LIF für die neue NIC
...
----
. Bei Einzelknotensystemen ist der neu hinzugefügte Port _e0b_. Bei HA-Paar-Bereitstellungen mit verwalteten Datenträgern ist der neu hinzugefügte Port _e0d_. Bei HA-Paar-Bereitstellungen mit Seitenblobs ist der neu hinzugefügte Port _e0e_. Verwenden Sie den Node-Name, nicht den VM-Name. Finden Sie den Node-Name, indem Sie  `node show` ausführen.
+
[source, cli]
----
