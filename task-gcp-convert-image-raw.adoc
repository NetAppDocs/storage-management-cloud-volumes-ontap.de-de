...
summary: Konvertieren Sie ein exportiertes Image über die Google Cloud-Befehlszeile in das disk.raw-Format. 
---
...
[role="lead"]
Das Image, das zum Bereitstellen neuer Instanzen, Upgrades oder in vorhandenen Images verwendet wird, wird mit den Clients geteilt durch https://mysupport.netapp.com/site/products/all/details/cloud-volumes-ontap/downloads-tab["die NetApp Support Site (NSS)"^] .  Der signierte Digest und die Zertifikate können über das NSS-Portal heruntergeladen werden.  Stellen Sie sicher, dass Sie den Digest und die Zertifikate für die richtige Version herunterladen, die dem vom NetApp Support freigegebenen Image entspricht.  Beispielsweise verfügen 9.13.0-Images über einen signierten 9.13.0-Digest und Zertifikate, die auf NSS verfügbar sind.
...
* Erstellen von Image-Exportaufgaben
...
Die bevorzugte Methode zum Exportieren eines Bildes in den Cloud-Speicher ist die Verwendung des https://cloud.google.com/sdk/gcloud/reference/compute/images/export["Befehl „gcloud compute images export“"^] .  Dieser Befehl nimmt das bereitgestellte Bild und konvertiert es in eine disk.raw-Datei, die getarnt und gzippt wird.  Die generierte Datei wird unter der Ziel-URL gespeichert und kann anschließend zur Überprüfung heruntergeladen werden.
...
NOTE: Weitere Informationen zum Exportieren eines Bildes über Google Cloud finden Sie im https://cloud.google.com/compute/docs/images/export-image#console["Google Cloud-Dokument zum Exportieren eines Bildes"^] .
