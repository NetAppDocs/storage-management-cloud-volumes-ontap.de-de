...
** Das mit dem Console Agent verknüpfte Dienstkonto https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html["sollte über die erforderlichen Berechtigungen verfügen"^].
...
. *Standort & Konnektivität*: Wählen Sie die Google Cloud-Region und -Zone für Ihr System, wählen Sie eine Firewall-Richtlinie und bestätigen Sie die Netzwerkverbindung zu Google Cloud storage für die Daten-Tiering.
+
In der folgenden Tabelle werden die Felder beschrieben, für die Sie möglicherweise Anleitungen benötigen:
...
. *Vorkonfigurierte Pakete*: Wählen Sie eines der Pakete aus, um schnell ein Cloud Volumes ONTAP-System bereitzustellen, oder klicken Sie auf *Eigene Konfiguration erstellen*. Die vorkonfigurierten Pakete variieren je nach gewählter Cloud Volumes ONTAP Version. Zum Beispiel zeigt die Console für Cloud Volumes ONTAP 9.18.1 und höher Pakete mit C3-VMs, einschließlich Hyperdisk Balanced-Festplatten, an. Sie können die Konfigurationen, wie IOPS- und Durchsatzparameter, basierend auf Ihren Workload-Anforderungen anpassen.
+
...
+
.. Aktivieren Sie *Flash Cache* oder wählen Sie *Normal* oder *High* Schreibgeschwindigkeit, wenn Sie es benötigen.
+
Erfahren Sie mehr über  https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-gcp.html#flash-cache-support["Flash-Cache"^] und link:concept-write-speed.html["Schreibgeschwindigkeit"].
...
CAUTION: Nach Abschluss des Bereitstellungsprozesses dürfen die systemgenerierten Cloud Volumes ONTAP Konfigurationen im Google Cloud-Portal, wie zum Beispiel die System-Tags und die in den Google Cloud-Ressourcen festgelegten Labels, nicht verändert werden. Jegliche Änderungen an diesen Konfigurationen können zu unerwartetem Verhalten oder Datenverlust führen.
...
. *Systemdetails*: Es wird ein Projekt ausgewählt, ein Clustername angegeben, optional ein Service Account ausgewählt, optional Bezeichnungen hinzugefügt und anschließend Anmeldeinformationen angegeben.
+
In der folgenden Tabelle werden die Felder beschrieben, für die Sie möglicherweise Anleitungen benötigen:
...
| Google Cloud Projekt und Marketplace Abonnement  a| 
Wählen Sie das Google-Projekt und das Abonnement aus, das Sie für dieses System verwenden möchten. Dies ist das Dienstkonto, das Sie für die NetApp Console eingerichtet haben. Weitere Informationen finden Sie unter link:https://docs.netapp.com/us-en/bluexp-setup-admin/task-quick-start-connector-google.html["Konsoleneinrichtungsdokumente"^]. Die NetApp Console verwendet das Projekt zur Erstellung des Cloud Volumes ONTAP Systems und das Abonnement zur Bestimmung von Abrechnung und Nutzung. Wenn Sie in der Dropdown-Liste keine weiteren Projekte sehen, haben Sie das Dienstkonto noch nicht mit anderen Projekten verknüpft. Rufen Sie die Google Cloud Console auf, öffnen Sie den IAM-Service und wählen Sie das Projekt aus. Fügen Sie das Dienstkonto mit der Rolle, die Sie für die NetApp Console verwenden, zu diesem Projekt hinzu. Sie müssen diesen Schritt für jedes Projekt wiederholen.

Klicken Sie auf *Abonnement hinzufügen*, um die ausgewählten Anmeldeinformationen einem Abonnement zuzuordnen.

Um ein Cloud Volumes ONTAP System mit nutzungsbasierter Abrechnung zu erstellen, ist die Auswahl eines Google Cloud-Projekts erforderlich, das mit einem Abonnement für Cloud Volumes ONTAP aus dem Google Cloud Marketplace verknüpft ist. Siehe das Dokument zur Einrichtung der Console https://docs.netapp.com/us-en/bluexp-setup-admin/task-adding-gcp-accounts.html["Verknüpfen eines Marktplatz-Abonnements mit Google Cloud-Anmeldeinformationen"^].
...
| Etiketten | Labels sind Metadaten für Ihre Google Cloud-Ressourcen.  Die Konsole fügt die Labels dem Cloud Volumes ONTAP -System und den mit dem System verknüpften Google Cloud-Ressourcen hinzu.  Sie können beim Erstellen eines Systems bis zu vier Beschriftungen über die Benutzeroberfläche hinzufügen und nach der Erstellung weitere hinzufügen.  Beachten Sie, dass die API Sie beim Erstellen eines Systems nicht auf vier Labels beschränkt.  Informationen zu Etiketten finden Sie unter https://cloud.google.com/compute/docs/labeling-resources["Google Cloud-Dokumentation: Kennzeichnungsressourcen"^] . 


| ONTAP Benutzername und Passwort | Dies sind die Anmeldeinformationen für das Cloud Volumes ONTAP Clusteradministratorkonto.  Sie können diese Anmeldeinformationen verwenden, um über ONTAP System Manager oder die ONTAP CLI eine Verbindung zu Cloud Volumes ONTAP herzustellen.  Behalten Sie den Standardbenutzernamen _admin_ bei oder ändern Sie ihn in einen benutzerdefinierten Benutzernamen. 
|===
...
. *Bereitstellungstopologie*: Es kann zwischen mehreren Verfügbarkeitszonen (AZ) (empfohlen) oder einer einzelnen AZ für die HA-Konfiguration gewählt werden.
+
link:concept-ha-google-cloud.html["Erfahren Sie mehr über HA-Bereitstellungsmodelle"^] .

. *Netzwerkkonfiguration und Konnektivität*: Die relevanten Regionen und Zonen werden ausgewählt. Vier verschiedene VPCs für die HA-Konfiguration, jeweils ein Subnetz in jeder VPC und anschließend eine Firewall-Richtlinie werden ausgewählt.
+

NOTE: Cloud Volumes ONTAP 9.19.1 und höher unterstützen link:concept-ha-google-cloud.html["Gemeinsam genutzte Datenträger für Single AZ HA-Konfigurationen"], wodurch die Mediator-Instanz optional wird. In dieser Konfiguration ist es nicht erforderlich, eine Mediator-Zone, die entsprechende VPC und das Subnetz für Datenreplizierung anzugeben.
...
+
Wenn Sie eines der Pakete auswählen, müssen Sie lediglich ein Volume angeben und anschließend die Konfiguration prüfen und genehmigen. Die vorkonfigurierten Pakete variieren je nach gewählter Cloud Volumes ONTAP Version und Konfigurationstypen. Wenn eine Bereitstellung in einer einzelnen AZ für Cloud Volumes ONTAP 9.19.1 oder höher ausgewählt wird, sind vorkonfigurierte Pakete mit C3-Series Maschinen und Hyperdisk Balanced Disks sichtbar. Für frühere Versionen und mehrere AZs können weiterhin n2-standard Maschinen und Persistent Disks ausgewählt werden. Die Konfigurationen, wie IOPS- und Durchsatzparameter, lassen sich entsprechend den Workload-Anforderungen anpassen.
