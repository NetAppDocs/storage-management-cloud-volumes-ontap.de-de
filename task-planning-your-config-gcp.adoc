...
Maschinentyp:: Die unterstützten Maschinentypen in der link:reference-configs-gcp.html["Versionshinweise zu Cloud Volumes ONTAP"^] können betrachtet werden, anschließend stehen bei Google Details zu jedem unterstützten Maschinentyp zur Verfügung. Die Workload-Anforderungen lassen sich mit der Anzahl der vCPUs und dem Arbeitsspeicher des Maschinentyps abgleichen. Zu beachten ist, dass jeder CPU-Kern die Netzwerkleistung erhöht.
+
--
