...
NOTE: Ab Cloud Volumes ONTAP 9.15.0P1 werden Azure Page Blobs für neue Hochverfügbarkeitspaar-Bereitstellungen nicht mehr unterstützt. Bei der Nutzung von Azure Page Blobs in bestehenden Hochverfügbarkeitspaar-Bereitstellungen besteht die Möglichkeit, auf neuere VM-Instanztypen der Edsv4-Serie und Edsv5-Serie zu migrieren. link:link:reference-configs-azure.html#ha-pairs["Erfahren Sie mehr über unterstützte Konfigurationen in Azure"].
...
NOTE: Cloud Volumes ONTAP verwendet Load Balancer im Aktiv/Passiv-Modus für Probes. In Azure antwortet nur der aktive Knoten auf eine Probe. Der passive Knoten wird möglicherweise als inaktiv mit einem X angezeigt. Dies ist das erwartete Verhalten und kein Grund zur Sorge.

.Weiterführender Link
link:task-azure-high-availability-mode.html["Aktivieren Sie den Hochverfügbarkeitsmodus für Cloud Volumes ONTAP in Azure"^]
