...
image:diagram_ha_azure.png["Ein konzeptionelles Bild, das einen Load Balancer zeigt, der den eingehenden Datenverkehr von der NetApp Konsole und den Clients verwaltet, zwei Cloud Volumes ONTAP Knoten in einem VMSS Flex, SSD-Festplatten für Startdaten und Page Blobs für Kundendaten."]

Beachten Sie Folgendes zu den Azure-Komponenten, die die NetApp Konsole für Sie bereitstellt:
